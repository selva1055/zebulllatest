...
export class Datafeedbase implements IExternalDatafeed, IDatafeedQuotesApi, IDatafeedChartApi {
	...
	private readonly _sharedataservice :ZebuodrGentrService;
	private readonly _websocketServ :WebsocketService;

	protected constructor(datafeedURL: string, quotesProvider: IQuotesProvider, requester: Requester, updateFrequency: number = 10 * 1000,service :ZebuodrGentrService,websocketserv:WebsocketService) {
		this._datafeedURL = datafeedURL;
		this._requester = requester;
		this._websocketServ=websocketserv;
		this._historyProvider = new HistoryProvider(datafeedURL, this._requester, service);
		this._quotesProvider = quotesProvider;
		this._sharedataservice =service;

		this._dataPulseProvider = new DataPulseProvider(this._historyProvider, updateFrequency,this._sharedataservice,this._websocketServ);
	...
	public searchSymbols(userInput: string, exchange: string, symbolType: string, onResult: SearchSymbolsCallback): void {
		if (this._configuration.supports_search) {
			...
			this._send<UdfSearchSymbolsResponse>('search', params)
				.then((response: UdfSearchSymbolsResponse ) => {
	...
	public resolveSymbol(symbolName: string, onResolve: ResolveCallback, onError: ErrorCallback): void {
		...
		if (!this._configuration.supports_group_request) {
			...
			this._send<ResolveSymbolResponse >('symbols', params)
				.then((response: ResolveSymbolResponse) => {
					if (response.s !== undefined) {
